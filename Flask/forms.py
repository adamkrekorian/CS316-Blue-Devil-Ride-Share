...
from wtforms import Form, BooleanField, StringField, IntegerField, PasswordField, validators, DateField, SubmitField, SelectField, ValidationError, DecimalField
from wtforms_components import TimeField
from wtforms.validators import InputRequired, Length, NumberRange, Regexp, Email
...
class GreaterThan(object):  # --> Change to 'GreaterThan'
    ...
    def __call__(self, form, field):
        ...
        if field.data <= other.data:  #  --> Change to <= from !=
            ...
            message = self.message
            if message is None:
                message = field.gettext('Latest departure date/time must be greater than earliest departure date/time')
...
class ListRideFormFactory(FlaskForm):
    ...
    earliest_departure = DateTimeLocalField("Earliest Date/Time of Departure", validators=[InputRequired(message='Please enter the earliest date/time of departure')], format='%Y-%m-%dT%H:%M')
    ...
    def validate_startdate_field(form, field):
        if earliest_departure.data < datetime.now():
            raise ValidationError("Earliest departure date/time must not be earlier than start date.")
